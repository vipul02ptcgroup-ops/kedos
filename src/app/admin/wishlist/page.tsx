...
import { subscribeWishlistItems, summarizeWishlist, WishlistAdminSummary } from '@/lib/wishlist';

export default function AdminWishlistPage() {
  const [products, setProducts] = useState<Product[]>([]);
  const [summary, setSummary] = useState<WishlistAdminSummary[]>([]);
  const [selected, setSelected] = useState<(WishlistAdminSummary & { product: Product | null }) | null>(null);

  useEffect(() => {
    const unsubProducts = subscribeProducts(setProducts);
    const unsubWishlist = subscribeWishlistItems((items) => setSummary(summarizeWishlist(items)));
    return () => {
      unsubProducts();
      unsubWishlist();
    };
  ...
  return (
    <AdminLayout>
      <div className="p-6">
        <div className="mb-6">
          <h1 className="font-display text-2xl text-slate-800">Wishlist</h1>
          <p className="text-sm text-slate-500 font-body">{totalAdds} total wishlist adds</p>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  {['Product', 'Category', 'Wishlist Adds', 'Users', 'Action'].map((h) => (
              ...
              <tbody className="divide-y divide-slate-50">
                {rows.map((row) => (
                  <tr key={row.productId} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-4">
                      <span className="inline-flex items-center gap-1.5 text-sm font-medium text-blush-600 font-body">
                        <Heart size={14} className="fill-blush-500 text-blush-500" />
                        {row.totalAdds}
                      </span>
                    </td>
                ...
                {rows.length === 0 && (
                  <tr>
                    <td colSpan={5} className="px-4 py-8 text-center text-sm text-slate-500 font-body">No wishlist data yet.</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <div className="fixed inset-0 z-50 flex justify-end" style={{ background: 'rgba(0,0,0,0.4)' }} onClick={() => setSelected(null)}>
          <div className="bg-white w-full max-w-md h-full overflow-y-auto shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              <h2 className="font-display text-xl text-slate-800">Users Who Wishlisted</h2>
